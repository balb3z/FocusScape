...
CREATE INDEX room_players_room_last_seen_idx ON public.room_players (room_id, last_seen DESC);
CREATE INDEX room_players_user_room_idx ON public.room_players (user_id, room_id);
...
CREATE POLICY "Anyone authenticated can view tables" ON public.room_tables FOR SELECT TO authenticated USING (true);
CREATE POLICY "Users can create tables" ON public.room_tables FOR INSERT TO authenticated WITH CHECK (auth.uid() = creator_id);
CREATE POLICY "Creators can update their tables" ON public.room_tables FOR UPDATE TO authenticated USING (auth.uid() = creator_id) WITH CHECK (auth.uid() = creator_id);
CREATE POLICY "Creators can delete their tables" ON public.room_tables FOR DELETE TO authenticated USING (auth.uid() = creator_id);
